...
function predecir(){
  ...
  setLoad(true);
  setTimeout(()=>{
    setRes({...calc(s1,s2,+q1,+qx,+q2,+hj,+hw1,+hw2),t1,t2});
    setLoad(false);
  },300);
}
...
<div>
<select value={t1} onChange={e=>{setT1(e.target.value);setRes(null);}} style={sel}>{teams.map(t=><option key={t} value={t}>{T[t].fl} {t}</option>)}</select>
<div style={{fontSize:9,color:P,marginTop:3}}>{sn[s1.s]} ELO {s1.e}</div>
{f1.n>0&&<div style={{display:"flex",gap:2,marginTop:3}}>{f1.fm.map((f,i)=><FT key={i} r={f}/>)}<span style={{fontSize:9,color:"#64748b",marginLeft:3}}>{f1.pts}pts</span></div>}
</div>
...
<div>
<select value={t2} onChange={e=>{setT2(e.target.value);setRes(null);}} style={sel}>{teams.map(t=><option key={t} value={t}>{T[t].fl} {t}</option>)}</select>
<div style={{fontSize:9,color:P,marginTop:3}}>{sn[s2.s]} ELO {s2.e}</div>
{f2.n>0&&<div style={{display:"flex",gap:2,marginTop:3}}>{f2.fm.map((f,i)=><FT key={i} r={f}/>)}<span style={{fontSize:9,color:"#64748b",marginLeft:3}}>{f2.pts}pts</span></div>}
</div>
</div>
