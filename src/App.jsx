...
function calc(s1,s2,q1,qx,q2,hj,hw1,hw2){
...
const pr=t1!==t2?calc(s1,s2,+q1,+qx,+q2,+hj,+hw1,+hw2):null;
const cC=pr?.cf==="ALTA"?G:pr?.cf==="MEDIA"?Y:"#ff6060";
...
return(
<div style={{minHeight:"100vh",background:C,fontFamily:"Inter,sans-serif",color:"#e2e8f0",maxWidth:600,margin:"0 auto",padding:12}}>
...
<div><select value={t1} onChange={e=>setT1(e.target.value)} style={sel}>{teams.map(t=><option key={t} value={t}>{T[t].fl} {t}</option>)}</select><div style={{fontSize:9,color:P,marginTop:3}}>{sn[s1.s]} ELO {s1.e}</div></div>
...
<div><select value={t2} onChange={e=>setT2(e.target.value)} style={sel}>{teams.map(t=><option key={t} value={t}>{T[t].fl} {t}</option>)}</select><div style={{fontSize:9,color:P,marginTop:3}}>{sn[s2.s]} ELO {s2.e}</div></div>
</div>
{pr?.al&&<div style={{background:"#ff303010",borderRadius:8,padding:10,marginBottom:12,border:"2px solid #ff6060"}}><div style={{fontSize:9,color:"#ff6060",fontWeight:800,marginBottom:3}}>ALERTA BLOQUE</div><div style={{fontSize:11,color:"#fca5a5"}}>Patron confirmado: España 0-0 Cabo Verde, Belgica 1-1 Egipto, Brasil 1-1 Marruecos, Iran 2-2 NZ. Usa Doble Oportunidad.</div></div>}
...
<div><div style={{fontSize:8,color:Y,marginBottom:2}}>Emp</div><input type="number" min="0" placeholder="0" style={inp}/></div>
...
<span style={{fontSize:10,padding:"2px 8px",borderRadius:12,fontWeight:700,color:cC,background:cC+"12",border:`1px solid ${cC}30`}}>{pr.cf}</span>
</div>
...
<div style={{textAlign:"center",flex:1}}><div style={{fontSize:28}}>{s1.fl}</div><div style={{fontSize:11,fontWeight:700,marginTop:3}}>{t1}</div></div>
<div style={{textAlign:"center"}}><div style={{fontSize:34,fontWeight:900,letterSpacing:4,color:"#fff"}}>{pr.sh}-{pr.sa}</div><div style={{fontSize:9,color:"#4a5568"}}>l {pr.l1}-{pr.l2}</div></div>
<div style={{textAlign:"center",flex:1}}><div style={{fontSize:28}}>{s2.fl}</div><div style={{fontSize:11,fontWeight:700,marginTop:3}}>{t2}</div></div>
</div>
<Fi l={"1 "+t1} v={pr.pH} c={G}/>
<Fi l="X Empate" v={pr.pD} c={Y}/>
<Fi l={"2 "+t2} v={pr.pA} c={B}/>
</div>
<div style={card}>
<div style={{fontSize:9,color:"#4a5568",letterSpacing:2,marginBottom:12}}>MERCADOS</div>
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
<div style={{background:C,borderRadius:8,padding:10,border:"1px solid #1e2a3a"}}><div style={{fontSize:9,color:"#4a5568",marginBottom:8}}>DOBLE OPORTUNIDAD</div><Fi l="1X" v={pr.hx} c={G}/><Fi l="X2" v={pr.x2} c={B}/><Fi l="No Emp" v={pr.ne} c={Y}/></div>
<div style={{background:C,borderRadius:8,padding:10,border:"1px solid #1e2a3a"}}><div style={{fontSize:9,color:"#4a5568",marginBottom:8}}>GOLES</div><Fi l="+1.5" v={pr.m15} c={G}/><Fi l="+2.5" v={pr.m25} c={B}/><Fi l="BTTS" v={pr.btts} c={P}/></div>
</div>
<div style={{padding:"10px 12px",borderRadius:8,background:"#00ff8808",border:"1px solid #00ff8825"}}><div style={{fontSize:9,color:G,fontWeight:800,marginBottom:3}}>MEJOR APUESTA</div><div style={{fontSize:13,color:"#e2e8f0",fontWeight:600}}>{pr.mj}</div></div>
</div>
</div>}
